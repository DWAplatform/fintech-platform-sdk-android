 ...
 */
class EnterpriseInfoActivity : AppCompatActivity(), EnterpriseInfoContract.Navigation {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        intent.extras?.getString("hostname")?.let { hostname ->
            intent.extras?.getParcelable<DataAccount>("dataAccount")?.let { dataAccount ->
                val frag = EnterpriseInfoFragment.newInstance(hostname, dataAccount)
                supportFragmentManager
                        .beginTransaction()
                        .add(R.id.contentContainer, frag, EnterpriseInfoFragment::class.java.canonicalName)
                        .commit()
            }
        }
    }
    ...
    override fun backFromBusinessInfo() {
        onBackPressed()
    }
}